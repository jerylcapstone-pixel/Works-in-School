...
// Default SQL query to fetch scheduled appointments
...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    // Append search criteria to the SQL query
    $sql .= "AND (s.Sname LIKE '%$search%' OR u.Fname LIKE '%$search%' OR u.Lname LIKE '%$search%' OR stylist.Fname LIKE '%$search%' OR stylist.Lname LIKE '%$search%' OR DATE_FORMAT(a.date, '%Y-%m-%d %h:%i:%s %p') LIKE '%$search%' OR a.total_payment LIKE '%$search%' OR a.hairstyle LIKE '%$search%' OR a.hair_color LIKE '%$search%') ";
}
...
} else if ($user_type == 'admin' || $user_type == 'sec') {
    $sql .= ""; // No additional condition for admin and sec
} else {
                    ...
                    <ul class="navbar-nav ms-auto me-0">
                        ...
                        <li class="nav-item dropdown active">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a class="dropdown-item active" href="scheduled_appointments.php">Scheduled Appointments</a></li>
    ...
    <div class="container mt-4">
        ...
        <form method="GET">
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Search..." name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>">
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th>Hair Style</th>
                    <th>Hair Color</th>
                    <?php if ($_SESSION['user_type'] == 'stylist') { ?>
                    <th>Actions</th>
                    <?php } ?>
                </tr>
            </thead>
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <?php if ($_SESSION['user_type'] == 'stylist') { ?>
                        <td>
                            <form action="reschedule_sappointment.php" method="post" style="display:inline;">
                                <input type="hidden" name="app_id" value="<?php echo htmlspecialchars($row['app_id']); ?>">
                                <button type="submit" class="btn btn-warning">Reschedule</button>
                            </form>
                            <form method="post" action="complete_appointment.php" style="display:inline;">
                                <input type="hidden" name="app_id" value="<?php echo htmlspecialchars($row['app_id']); ?>">
