...
// Check if the user is logged in and has the required privileges

// Check if the user is logged in and has the required privileges
if (!isset($_SESSION['username']) || $_SESSION['user_type'] == 'client') {
    // Redirect clients to another page
    header("Location: home.php"); // Replace "another_page.php" with the page you want to redirect clients to
    exit();
}
   ...
   <nav class="navbar navbar-expand-lg navbar-dark custom-navbar sticky-top">
    ...
    <div class="container mt-4">
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3">
            ...
            // Fetch archived services data from the database
            $sql = "SELECT * FROM services WHERE status = 'archived'"; // Assuming you have an 'archived' column
            $result = $conn->query($sql);

            // Check if there are any archived services available
            if ($result->num_rows > 0) {
                // Loop through each archived service
    ...
    <?php if ($_SESSION['user_type'] == 'admin' || $_SESSION['user_type'] == 'sec'): ?>
        <a href="restore_service.php?id=<?php echo $row['service_id']; ?>" class="btn btn-primary">Restore</a>
    <?php endif; ?>
</div>
                ...
                // If no archived services are available, display a message
                ?>
                <div class="col-md-12">
                    <p>No archived services available.</p>
                </div>
                <?php
            }
