...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = 'Active'; // New courses are always set to Active
    ...
    if (empty($course_name) || $credits <= 0 || !$department_id) {
        ...
        try {
            ...
            // Get next course_id to generate code before insert
            $stmt = $pdo->query("SELECT COALESCE(MAX(course_id), 0) + 1 as next_id FROM courses");
            $result = $stmt->fetch();
            $next_id = $result['next_id'];
            
            // Generate auto-increment course code: COURSE-001, COURSE-002, etc.
            $course_code = 'COURSE-' . str_pad($next_id, 3, '0', STR_PAD_LEFT);
            ...
            // Build SQL dynamically based on whether major_id column exists
            if ($major_column_exists) {
                $sql = "INSERT INTO courses (course_code, course_name, department_id, credits, course_type, description, status, major_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $params = [$course_code, $course_name, $department_id, $credits, $course_type, $description ?: null, $status, $major_id];
        } else {
                $sql = "INSERT INTO courses (course_code, course_name, department_id, credits, course_type, description, status)
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                $params = [$course_code, $course_name, $department_id, $credits, $course_type, $description ?: null, $status];
            }
            $stmt = $pdo->prepare($sql);
            if ($stmt->execute($params)) {
                $course_id = $pdo->lastInsertId();
                
                // If program is selected, create program_course relationship
                if ($program_id) {
                ...
                $pdo->commit();
                header('Location: ' . BASE_URL . 'modules/curriculum/index.php?tab=courses&success=Course added successfully with code: ' . $course_code);
                exit;
            } else {
                $pdo->rollBack();
...
<div class="page-container">
    <div class="page-header">
        <h1><i class="fas fa-book-open"></i> Add Course</h1>
        ...
        <form method="POST" action="" class="form">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <small class="text-muted">Course code will be auto-generated (e.g., COURSE-001, COURSE-002)</small>
                </div>
                <div class="form-group">
                    <label>Course Name <span class="required">*</span></label>
                    <input type="text" name="course_name" required class="form-control" 
                           value="<?php echo sanitizeOutput($_POST['course_name'] ?? ''); ?>">
                </div>
            </div>
                        ...
                        <?php if (!empty($programs)): ?>
                            <?php foreach ($programs as $prog): ?>
                                ...
                                <option value="<?php echo $progId; ?>"
                                        ...
                                        <?php echo (($_POST['program_id'] ?? '') == $progId) ? 'selected' : ''; ?>>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="credits" required class="form-control" min="1" max="6"
                           value="<?php echo sanitizeOutput($_POST['credits'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="course_type" class="form-control" required>
                        <?php
                        $course_type = $_POST['course_type'] ?? 'Lecture';
                        if (is_string($course_type)) {
                            $course_type = ucfirst(strtolower($course_type));
                        }
                        $course_types = [
                            'Lecture' => 'Lecture',
                        ...
                        ?>
                            <option value="<?php echo $value; ?>" <?php echo ($course_type === $value) ? 'selected' : ''; ?>>
