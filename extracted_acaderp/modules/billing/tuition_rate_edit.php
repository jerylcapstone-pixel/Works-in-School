...
// Get tuition rate details
$stmt = $pdo->prepare("SELECT * FROM tuition_rates WHERE rate_id = ?");
...
// Get current values
$current_lecture_rate = 0;
$current_laboratory_rate = 0;
if ($lecture_rate_column_exists && isset($rate['lecture_rate'])) {
    $current_lecture_rate = (float)$rate['lecture_rate'];
} elseif (isset($rate['amount'])) {
    $current_lecture_rate = (float)$rate['amount']; // Fallback to old amount column
}

if ($laboratory_rate_column_exists && isset($rate['laboratory_rate'])) {
    $current_laboratory_rate = (float)$rate['laboratory_rate'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($lecture_rate_column_exists && $laboratory_rate_column_exists) {
            // If status is Active and program changed or status changed to Active, deactivate other active rates
            if ($status === 'Active' && ($rate['program_id'] != $program_id || $rate['status'] !== 'Active')) {
                if ($program_id) {
                    $stmt = $pdo->prepare("UPDATE tuition_rates SET status = 'Inactive' 
                                          WHERE program_id = ? AND status = 'Active' AND rate_id != ?");
                    $stmt->execute([$program_id, $rate_id]);
                } else {
                    // For general education (NULL program_id)
                    $stmt = $pdo->prepare("UPDATE tuition_rates SET status = 'Inactive' 
                                          WHERE program_id IS NULL AND status = 'Active' AND rate_id != ?");
                    $stmt->execute([$rate_id]);
                }
            }
            
            // Check for duplicate active rates (excluding current)
            if ($status === 'Active') {
                if ($program_id) {
                    $check_stmt = $pdo->prepare("SELECT COUNT(*) FROM tuition_rates 
                                                WHERE program_id = ? AND status = 'Active' AND rate_id != ?");
                    $check_stmt->execute([$program_id, $rate_id]);
                } else {
                    $check_stmt = $pdo->prepare("SELECT COUNT(*) FROM tuition_rates 
                                                WHERE program_id IS NULL AND status = 'Active' AND rate_id != ?");
                    $check_stmt->execute([$rate_id]);
                }
                if ($check_stmt->fetchColumn() > 0) {
                    $rate_type = $program_id ? 'this program' : 'General Education';
                    throw new Exception("An active tuition rate already exists for $rate_type. Please deactivate it first or change this rate to Inactive.");
                }
            }
            
            // Update rate
            $stmt = $pdo->prepare("UPDATE tuition_rates SET 
                                  program_id = ?, 
                                  lecture_rate = ?, 
                                  laboratory_rate = ?, 
                                  effective_date = ?, 
            ...
            $stmt->execute([
                ...
                $status, 
                $description,
                $lecture_rate, // Update amount column for backward compatibility
                $rate_id
            ]);
        } else {
        ...
        $success = true;
        header('Location: ' . BASE_URL . 'modules/billing/tuition_rates.php?success=' . urlencode('Tuition rate updated successfully.'));
        exit;
...
<div class="page-container">
    <div class="page-header">
        <h1><i class="fas fa-edit"></i> Edit Tuition Rate</h1>
        ...
        <div class="card">
            ...
            <div class="form-row">
                <div class="form-group" style="flex: 2;">
                    ...
                    <select id="program_id" name="program_id" class="form-control">
                        <option value="" <?php echo !$rate['program_id'] ? 'selected' : ''; ?>>-- General Education / No Program --</option>
                        <?php 
                        $current_dept = '';
                        ...
                        ?>
                            <option value="<?php echo $prog['program_id']; ?>" 
                                    <?php echo $rate['program_id'] == $prog['program_id'] ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <input type="number" id="lecture_rate" name="lecture_rate" class="form-control" 
                               step="0.01" min="0.01" 
                               value="<?php echo $current_lecture_rate > 0 ? number_format($current_lecture_rate, 2, '.', '') : ''; ?>" required>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <input type="number" id="laboratory_rate" name="laboratory_rate" class="form-control" 
                               step="0.01" min="0" 
                               value="<?php echo $current_laboratory_rate > 0 ? number_format($current_laboratory_rate, 2, '.', '') : '0'; ?>">
                    </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="effective_date" name="effective_date" class="form-control" 
                           value="<?php echo date('Y-m-d', strtotime($rate['effective_date'])); ?>" required min="<?php echo date('Y-m-d'); ?>">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control">
                        <option value="Active" <?php echo ($rate['status'] ?? 'Active') === 'Active' ? 'selected' : ''; ?>>Active</option>
                        <option value="Inactive" <?php echo ($rate['status'] ?? 'Active') === 'Inactive' ? 'selected' : ''; ?>>Inactive</option>
                    </select>
                    <small class="text-muted">Active rates will be used for billing</small>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group" style="flex: 2;">
                    ...
                    <textarea id="description" name="description" class="form-control" rows="2"><?php echo sanitizeOutput($rate['description'] ?? ''); ?></textarea>
                </div>
            </div>
            
            <div class="form-actions">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Update Tuition Rate
                </button>
                <a href="<?php echo BASE_URL; ?>modules/billing/tuition_rates.php" class="btn btn-secondary">
                    Cancel
                </a>
